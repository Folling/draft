...
\diffref{default.allocator.general}
\change
Remove \tcode{is_always_equal} from class template \tcode{std::allocator}.
\rationale
\tcode{is_always_equal}, when added to \tcode{std::allocator},
causes the standard library to treat derived types as equal.
\effect
Valid \CppXVII{} code may fail to compile or may change meaning
in this revision of \Cpp{}.

\diffref{mem.poly.allocator.class}
\change
Remove \tcode{destroy} from class template \tcode{std::pmr::polymorphic_allocator}.
\rationale
This function is equivalent to the default implementation of \tcode{destroy}
in \tcode{allocator_traits}\iref{allocator.traits.members}.
\effect
Valid \CppXVII{} code may fail to compile or may change meaning
in this revision of \Cpp{}.
...
\change
Specify use of \tcode{explicit} in existing boolean conversion functions.
\rationale
...
\change
A name of file scope that is explicitly declared \tcode{const}, and not explicitly
...
\diffref{dcl.stc}
\change
In \Cpp{}, the \tcode{static} or \tcode{extern} specifiers can only be applied to names of objects or functions.\\
