...
\pnum
The declarations in a program appear in a number of scopes
...
ppearing in another scope which thereby contains $S$.
...
at that point.
A scope \defnx{intervenes}{intervene}
...
\item
A declaration inhabits the immediate scope at its locus\iref{basic.scope.pdecl}.
\item
...
\end{footnote},
equivalent\iref{temp.over.link} trailing \grammarterm{requires-clauses}
...
trailing \grammarterm{requires-clauses} (if any), and,
if both are non-static members,
the same \grammarterm{cv-qualifier}s (if any) and
...
\pnum
\begin{note}
After the declaration of a class member, the member name can be
found in the scope of its class.
\indextext{type!incomplete}%
...
variable declarations with the \tcode{extern} specifier at block scope
declare entities
...
\indextext{enumeration scope|see{scope, enumeration}}%
\indextext{scope!enumeration}

\pnum
Any declaration of an enumeration $E$ introduces
an \defnx{enumeration scope}{scope!enumeration}
...
\item
Any exported declaration \tcode{D} in \tcode{N}
...
\pnum
A class member name hidden by a name in a nested scope or
...
if the two declarations appeared in the same scope,
