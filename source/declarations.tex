...
ci = 4;                                 // error: attempt to modify \tcode{const}

int i = 2;                              // not cv-qualified
const int* cip;                         // pointer to \tcode{const int}
cip = &i;                               // OK: cv-qualified access path to unqualified
*cip = 4;                               // error: attempt to modify through ptr to \tcode{const}
...
\begin{codeblock}
typedef int& A;
const A aref = 3;   // error: lvalue reference to non-\tcode{const} initialized with rvalue
\end{codeblock}
...
whose type is
that of \tcode{\exposid{e}.$\tcode{m}_i$}\iref{expr.ref};
the referenced type is
the declared type of $\tcode{m}_i$ if that type is a reference type, or
the type of \tcode{\exposid{e}.$\tcode{m}_i$} otherwise.
The lvalue is a
...
\begin{note}
Two \grammarterm{using-declaration}{s} can introduce functions with the same
